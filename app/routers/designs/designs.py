...
from app.dependencies import User, authenticate
...
async def create_design(design: DesignCreate, db_session = Depends(get_db_session), user: User = Depends(authenticate)):

    if user.email != design.user_email:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only create designs for your own user account."
        )
    ...
    allowed = await permit.check(
        ...
        # resource info
        {
            "type": RESOURCE_NAME,
            "attributes": {
                "user_email": design.user_email  # additional example attribute
            }
        }
    )
    ...
    resource_instance_create = ResourceInstanceCreate(
        key= str(created_design.id),
        resource='design',
        attributes = {
            "creator": design.user_email,
            "user_email": design.user_email
        },
        tenant='default'
    )
...
async def delete_design(design_id: int, user=Depends(authenticate), db_session = Depends(get_db_session)):
    ...
    allowed = await permit.check(db_design.user_email, 'delete', f"design:{design_id}")
...
async def edit_design(editDesign: DesignEdit,user=Depends(authenticate), db_session = Depends(get_db_session)):
    ...
    allowed = await permit.check(editDesign.user_email, 'edit', f"design:{editDesign.id}")
    if not allowed:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized")
    
    db_design = (await db_session.execute(
    select(Design).where(
        Design.id == editDesign.id
    ))).scalars().first()
